...
def parse_cora():
    ...
    # read node feature in xx.content
    idx_features_labels = np.genfromtxt(f"{path}.content", dtype=np.dtype(str))  # <paper_id>\t<word_bag>\t<class_label>
    data_X = idx_features_labels[:, 1:-1].astype(np.float32)  # for cora, (2708, 1435)
    labels = idx_features_labels[:, -1]
    class_map = {x: i for i, x in enumerate(['Case_Based', 'Genetic_Algorithms', 'Neural_Networks',
                                            'Probabilistic_Methods', 'Reinforcement_Learning', 'Rule_Learning', 'Theory'])}
    data_Y = np.array([class_map[l] for l in labels])
    data_citeid = idx_features_labels[:, 0]           
    idx = np.array(data_citeid, dtype=np.dtype(str))   # paperid
    idx_map = {j: i for i, j in enumerate(idx)}        # paperid 2 idxInTrain
    
    edges_unordered = np.genfromtxt("{}.cites".format(path), dtype=np.dtype(str))
    edges = np.array(list(map(idx_map.get, edges_unordered.flatten()))).reshape(edges_unordered.shape)
    # upper is for node, following if for edge
