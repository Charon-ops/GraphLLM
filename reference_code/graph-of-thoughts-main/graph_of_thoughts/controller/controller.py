...
class Controller:
    ...
    def __init__(
        ...
        prompter: Prompter, parser: Parser,
        ...
        """
        self.lm, self.graph, self.prompter, self.parser = lm, graph, prompter, parser
        self.problem_parameters = problem_parameters
        self.logger = logging.getLogger(self.__class__.__module__)
        self.run_executed = False  # when run function is finished, run_executed will turn to True
    ...
    def run(self) -> None:
        ...
        # execute queue (in process sometimes there will be new operation to enqueue) util it's empty
        while len(execution_queue) > 0:
            # execute cur_operation
            ...
            # if cur_operation's son_operation could be executed, let it enqueue
            for operation in current_operation.successors:
                assert (operation in self.graph.operations
    ...
    def get_final_thoughts(self) -> List[List[Thought]]:
        ...
        return [operation.get_thoughts() for operation in self.graph.leaves]  # get final thoughts from leaves
    ...
    def output_graph(self, path: str) -> None:
        ...
        for operation in self.graph.operations:
            #### Prepare operation_serialized: dict
            ...
            ## c) 'compared_to_ground_truth' and 'problem_solved'
            ...
            #### Save operation_serialized
            output.append(operation_serialized)

        #### output[:-1] is list of `operation_serialized`, output[-1] is overhead
        output.append(
            {
